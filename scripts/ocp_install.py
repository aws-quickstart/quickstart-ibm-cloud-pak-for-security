...
class OCPInstall():
    ...
    def run_install(self):
        """Configures machine, install-config, and runs openshift-install.
        """
        ...
        try:
            ...
            install_config_template = open("/ibm/installDir/1AZ.yaml", "r").read()
            ...
            with open("/ibm/installDir/install-config.yaml", "w+") as icfg_file:
                icfg_file.write(install_config_template)
                icfg_file.close()

            logging.info("STARTING OCP INSTALL")

            install_ocp = "cd /ibm; sudo ./openshift-install create cluster --dir=/ibm/installDir --log-level=debug"
